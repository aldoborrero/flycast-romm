...
| 3   | Unit tests                                        | `go test -race ./...`                               | pass — session claims and their exclusions, ROM resolution, savestate naming, write settling, the Lua protocol against a fake script, pidfile reaping, the crash-loop limiter, every route against a fake controller |
