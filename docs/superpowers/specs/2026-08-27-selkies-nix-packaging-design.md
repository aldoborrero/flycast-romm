...
- **FFmpeg ceiling: ≤ 8.1** (`ffmpeg-sys-next 8.1` probes up to 8.1; the VAAPI path uses ~5.1-era API). nixpkgs default `ffmpeg` is **8.1.2** today (a patch of 8.1 → satisfies the major.minor probe). The real risk is a _future_ nixpkgs bump to 8.2/9.0; pin `ffmpeg` explicitly.
...
4. **Python floors** — met for `348bc4f` (`cryptography>=44` vs nixpkgs 49; `pyopenssl>=25` vs 26.3; `pylibsrtp>=0.10` vs 1.0). **A bump to `main` would _not_ be met** (`cryptography>=50`, `pyopenssl>=26`) → a `main` bump needs a `cryptography` override or nixpkgs advancing first. This is a concrete reason to hold at `348bc4f`.
5. **npm builds with gitignored lockfiles** — generate and pin; Vite 8 + React 19 trees are large.
6. **`main` vs `348bc4f` for WebRTC** — F3 may force a version bump _and_ pull in the signaling/STUN/TURN plane; scoped as its own step.
